...
function OccupationAnalysis() {
  ...
  const subMajorGroupsQuery = useSubMajorGroups(
    path[0]?.id ?? null,
    fromDate,
    toDate,
  );
  const minorGroupsQuery = useMinorGroups(
    path[1]?.id ?? null,
    fromDate,
    toDate,
  );
  const unitGroupsQuery = useUnitGroups(path[2]?.id ?? null, fromDate, toDate);
  const occupationGroupsQuery = useOccupationGroups(
    path[3]?.id ?? null,
    fromDate,
    toDate,
  );
  ...
  function isLevelSettled(levelIndex: number): boolean {
    switch (levelIndex) {
      case 0:
        return majorGroupQuery.isSuccess;
      case 1:
        return subMajorGroupsQuery.isSuccess;
      case 2:
        return minorGroupsQuery.isSuccess;
      case 3:
        return unitGroupsQuery.isSuccess;
      case 4:
        return occupationGroupsQuery.isSuccess;
      default:
        return false;
    }
  }
  ...
  useEffect(() => {
    ...
    path,
    majorGroupQuery.isSuccess,
    majorGroups,
    subMajorGroupsQuery.isSuccess,
    subMajorGroupsQuery.data,
    minorGroupsQuery.isSuccess,
    minorGroupsQuery.data,
    unitGroupsQuery.isSuccess,
    unitGroupsQuery.data,
    occupationGroupsQuery.isSuccess,
    occupationGroupsQuery.data,
  ]);
