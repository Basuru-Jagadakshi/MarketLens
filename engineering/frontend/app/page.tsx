...
const TRANSLATIONS = {
  en: {
    ...
    indModalSub: "Past year performance indices categorized by specific market industries",
...
export default function DashboardPage() {
  ...
  const [activePanel, setActivePanel] = useState<"OCCUPATION" | "INDUSTRY" | null>(null);
  ...
  const occTrendData = occAnalytics?.yearly_trend?.yearly_trend?.map((t) => ({
    year: String(t.year), vacancies: t.open_job_count,
  })) ?? [];
  ...
  const indExpData     = indAnalytics?.by_experience.experiences.map((e) => ({ label: e.name, value: e.open_job_count })) ?? [];
  const indProvinceData = indAnalytics?.by_province.provinces.map((p) => ({ name: p.province, value: p.open_job_count })) ?? [];
  const indEduData     = indAnalytics?.by_education.education_levels.map((e) => ({ label: e.level, value: e.open_job_count })) ?? [];
  ...
  if (isError) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <p className="text-sm text-red-500 font-medium">Failed to load dashboard. Please try again.</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      ...
      <div className="flex h-[calc(100vh-73px)]">
        ...
        <div className={`flex-1 overflow-y-auto transition-all duration-300 ${activePanel ? "xl:mr-0" : ""}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-8">
              <div className="bg-white p-6 rounded-xl shadow-sm">
                ...
                <div className="space-y-4">
                  {[{ label: "On-Site", share: onsitePct }, { label: "Remote", share: remotePct }].map((item, i) => (
                    <div key={i}>
                ...
                <div className="space-y-4">
                  {jobTypeData.map((jt, i) => {
                    ...
                    return (
                      <div key={i}>
                        <div className="flex justify-between text-xs mb-1 font-medium text-gray-600">
                          <span>{jt.type}</span><span className="font-mono">{pct}%</span>
                        </div>
                        <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-start bg-white shrink-0">
              <div>
                <h2 className="text-sm font-black text-gray-900">
                  {activePanel === "OCCUPATION" ? d.occModalTitle : d.indModalTitle}
                </h2>
                          ...
                          {topJobRoles.map((role, i) => (
                            <div key={i} className="flex items-center justify-between bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-xs font-bold text-gray-800">{role.job_role}</span>
                              </div>
                              <span className="text-[11px] font-black text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg">{role.open_job_count} open</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl">
                        ...
                        <div className="h-44">
                          <ResponsiveContainer>
                            <PieChart>
                              <Pie data={indProvinceData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={55} paddingAngle={2}>
